...
async def comprehensive_publishing_analysis(request: Dict):
    ...
    try:
        ...
        # 프로그램 랭킹 정보 (점수는 프롬프트에 포함하되 응답에서는 언급하지 않도록)
        program_ranking_text = ""
        if program_rankings:
            program_ranking_text = "\n**프로그램 추천 순위** (내부 분석용 - 응답에서는 점수 언급 금지):\n"
            for idx, prog in enumerate(program_rankings[:5], 1):
                program_ranking_text += f"{idx}. {prog['program']}"
                if prog.get('recommended', False):
                    program_ranking_text += " (강력 추천)"
                program_ranking_text += f" - 종합 {prog['overall_score']:.1f}점, 추천 {prog['recommendation_score']:.1f}점, 시간 적합도 {prog['time_suitability']:.1f}점, 타겟 매칭 {prog['demographic_match']:.1f}점\n"
        
        # LLM 프롬프트 생성 (간결하고 실용적인 큐레이션 전략)
        prompt = f"""당신은 파주시 출판단지 활성화를 위한 전문 큐레이션 기획자입니다.
데이터를 분석하여 큐레이션 기획자가 바로 활용할 수 있는 창의적이고 실행 가능한 전략을 제시해주세요.

**분석 날짜**: {date_label} ({date}, {weekday_name}) - {'주말' if is_weekend else '평일'}

**핵심 데이터**:
- 예상 방문인구: {predicted_visits:,.0f}명
- 예상 혼잡도: {crowd_level*100:.1f}% {'(혼잡)' if crowd_level > 0.7 else '(여유)' if crowd_level < 0.5 else '(보통)'}
- 최적 시간대: {optimal_time}
- 주요 타겟: {demographic_targeting.get('recommended_target', {}).get('age_group', 'N/A')} {demographic_targeting.get('recommended_target', {}).get('gender', 'N/A')}
- 주말/평일 비율: {weekend_ratio:.2f}배

{program_ranking_text}

**활성화 지표** (내부 분석용 - 응답에서는 점수 언급 금지):
- 접근성: {activation_scores.get('accessibility', 0):.1f}점 {'(우수)' if activation_scores.get('accessibility', 0) >= 70 else '(보통)' if activation_scores.get('accessibility', 0) >= 50 else '(낮음)'}
- 관심도: {activation_scores.get('interest', 0):.1f}점 {'(우수)' if activation_scores.get('interest', 0) >= 70 else '(보통)' if activation_scores.get('interest', 0) >= 50 else '(낮음)'}
- 잠재력: {activation_scores.get('potential', 0):.1f}점 {'(우수)' if activation_scores.get('potential', 0) >= 70 else '(보통)' if activation_scores.get('potential', 0) >= 50 else '(낮음)'}
- 활용도: {activation_scores.get('utilization', 0):.1f}점 {'(우수)' if activation_scores.get('utilization', 0) >= 70 else '(보통)' if activation_scores.get('utilization', 0) >= 50 else '(낮음)'}
- 출판단지 활성화 지수: {vitality_score:.1f}점

**🎯 분석 요구사항**:

1. **분석 요약** (100-150자): 해당 날짜의 활성화 상태를 큐레이션 관점에서 요약

2. **서술형 분석** (5-6개 문단, 각 100-200자):
   - 활성화 전망과 큐레이션 기회
   - 프로그램별 적합도 분석 및 추천
   - 날짜별 맞춤 큐레이션 전략
   - 타겟 고객 맞춤 큐레이션
...
3. **주요 강점** (3-5개): 큐레이션 강점

4. **개선 필요 영역** (3-5개): 개선 방안

5. **창의적 기회** (5-7개): 새로운 큐레이션 기회

6. **실행 추천** (7-10개): 바로 실행 가능한 추천

7. **실행 계획** (5단계): 단계별 계획

**응답 형식** (JSON):
{{
  "summary": "활성화 상태 요약 (100-150자)",
  "detailed_analysis": ["분석 1 (100-200자)", "분석 2", "분석 3", "분석 4", "분석 5", "분석 6"],
  "strengths": ["강점 1 (50-80자)", "강점 2", "강점 3", "강점 4", "강점 5"],
  "weaknesses": ["개선점 1 (50-80자)", "개선점 2", "개선점 3", "개선점 4", "개선점 5"],
  "opportunities": ["기회 1 (50-80자)", "기회 2", "기회 3", "기회 4", "기회 5", "기회 6", "기회 7"],
  "recommendations": ["추천 1 (50-80자)", "추천 2", "추천 3", "추천 4", "추천 5", "추천 6", "추천 7", "추천 8", "추천 9", "추천 10"],
  "action_plan": ["1단계: ... (50-80자)", "2단계: ...", "3단계: ...", "4단계: ...", "5단계: ..."]
}}

**중요 지침**:
- **점수/스코어 언급 금지**: 위의 지표들은 분석을 위한 것이며, 응답에서는 점수, 스코어, 점수값을 절대 언급하지 말 것. 프로그램 이름과 추천 이유만 제시 (예: "북토크 프로그램이 가장 적합", "오후 시간대 운영 권장")
- **기술 용어 금지**: "ML", "머신러닝", "예측 모델" 같은 기술 용어 사용 금지
- **자연스러운 표현**: "데이터 분석 결과", "예상 방문인구", "프로그램 적합도" 등 자연스러운 표현 사용
- **실용성 중심**: 큐레이션 기획자가 바로 실행할 수 있는 구체적 정보 중심 (시간대, 규모, 타겟, 마케팅 포인트 등)
- **간결성**: 불필요한 설명 제거, 핵심만 간결하게
